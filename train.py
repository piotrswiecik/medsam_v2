...
class MetricsTracker:
    ...
    def get_metrics(self):
        ...
        mean_f1 = (
            2 * mean_precision * mean_recall / (mean_precision + mean_recall + 1e-8)
        )
...
class MedSAMSyntax(nn.Module):
    ...
    def _extract_decoder_features(
        self, image_embedding, image_pe, sparse_embeddings, dense_embeddings
    ):
        """
        ...
        tokens = torch.cat(
            (output_tokens, sparse_embeddings), dim=1
        )  # [B, 7, 256] (2 sparse + 5 output)
    ...
    def forward(self, image, boxes):
        ...
        if isinstance(boxes, np.ndarray):
            boxes_torch = torch.as_tensor(
                boxes, dtype=torch.float32, device=image.device
            )
        else:
            boxes_torch = boxes
        ...
        for i in range(batch_size):
            ...
            with torch.no_grad():
                sparse_embeddings, dense_embeddings = self.prompt_encoder(
                    points=None, boxes=single_box, masks=None
                )  # ([B, 2, 256], [B, 256, 64, 64])
                image_pe = (
                    self.prompt_encoder.get_dense_pe()
                )  # [B, 256, 64, 64] TODO verify

            upscaled_features, iou_predictions = self._extract_decoder_features(
                single_image_embedding, image_pe, sparse_embeddings, dense_embeddings
            )
...
class CombinedLoss(nn.Module):
    ...
    def dice_loss(self, inputs, targets, smooth=1e-5):
        ...
        for c in range(1, self.num_classes):
            ...
            dice = (2.0 * intersection + smooth) / (
                input_c.sum() + target_c.sum() + smooth
            )
    ...
    def forward(self, inputs, targets):
        ...
        """
        ce_loss = F.cross_entropy(inputs, targets.long(), weight=self.class_weights)
...
def save_clahe_debug_samples(dataset, workdir, num_samples=3):
    """
    Save debug visualization of CLAHE preprocessing.

    Saves first N samples showing:
    - Original image (after resize, before CLAHE)
    - CLAHE-enhanced image
    - Final augmented image (after all transforms)

    Args:
        dataset: SyntaxDataset instance
        workdir: Path to save debug images
        num_samples: Number of samples to save (default: 3)
    """
    debug_dir = Path(workdir) / "clahe_debug"
    debug_dir.mkdir(parents=True, exist_ok=True)

    print(f"\n[INFO] Saving CLAHE debug samples to: {debug_dir}")

    for idx in range(min(num_samples, len(dataset))):
        # Load original image (without CLAHE)
        img_id = dataset.image_ids[idx]
        img_info = dataset.coco.loadImgs(img_id)[0]
        img_name = img_info["file_name"]
        img_path = dataset.data_root / dataset.split / "images" / img_name

        # Original (after resize)
        original = cv2.imread(str(img_path))
        original = cv2.cvtColor(original, cv2.COLOR_BGR2RGB)
        if original.shape[:2] != (dataset.img_size, dataset.img_size):
            original = cv2.resize(
                original,
        ...
        # CLAHE-enhanced
        clahe_enhanced = apply_clahe(original, CLAHE_CLIP_LIMIT, CLAHE_TILE_GRID_SIZE)

        # Get full pipeline output (with augmentations)
        sample = dataset[idx]
        final_tensor = sample["image"]  # normalized and augmented

        # Denormalize for visualization: reverse (x - 0.5) / 0.5 -> x * 0.5 + 0.5
        final_denorm = final_tensor.permute(1, 2, 0).numpy() * 0.5 + 0.5
        final_denorm = np.clip(final_denorm * 255, 0, 255).astype(np.uint8)

        # Create comparison grid
        fig, axes = plt.subplots(1, 3, figsize=(18, 6))

        axes[0].imshow(original)
        axes[0].set_title(f"Original\n{img_name}", fontsize=11)
        axes[0].axis("off")

        axes[1].imshow(clahe_enhanced)
        axes[1].set_title(
            f"CLAHE Enhanced\nclip={CLAHE_CLIP_LIMIT}, tile={CLAHE_TILE_GRID_SIZE}",
            fontsize=11,
        )
        axes[1].axis("off")

        axes[2].imshow(final_denorm)
        axes[2].set_title("Final (After Augmentations)", fontsize=11)
        axes[2].axis("off")

        plt.tight_layout()

        output_file = debug_dir / f"train_sample_{idx:02d}.png"
        plt.savefig(output_file, dpi=100, bbox_inches="tight")
        plt.close()

        print(f"   [INFO] Saved: {output_file.name}")

    print(
        f"[INFO] CLAHE debug samples saved (will be overwritten on next training run)\n"
    )
...
class SyntaxDataset:
    def __init__(
        ...
        if self.use_augmentations:
            self.augment = A.Compose(
                [
                    ...
                    A.OpticalDistortion(
                        distort_limit=self.augmentation_params[
        ...
        print(
            f"[INFO] Bbox scaling: {original_size} → {img_size} (×{self.scale_factor})"
        )
        if USE_CLAHE:
            print(
                f"[INFO] CLAHE preprocessing: ENABLED (clip_limit={CLAHE_CLIP_LIMIT}, tile_size={CLAHE_TILE_GRID_SIZE})"
            )
        else:
            print(f"[INFO] CLAHE preprocessing: DISABLED")
    ...
    def __getitem__(self, idx) -> dict[str, torch.Tensor | int | str]:
        ...
        # Apply CLAHE preprocessing (before augmentations)
        # TODO: Move to config
        if USE_CLAHE:
...
def _display_class_stats(stats, class_pixel_counts):
    ...
    for s in stats:
        ...
        print(
            f"{s['class_id']:<10} {s['pixel_count']:<15,} {s['percentage']:<12.4f}% {fg_str:<15}"
        )
    ...
    print(
        f"{'Foreground (all vessels):':<25} {foreground_pixels:,} ({100*foreground_pixels/total_pixels:.2f}%)"
    )
    print(
        f"{'Background:Foreground ratio:':<25} {background_pixels/max(foreground_pixels,1):.2f}:1"
    )
    ...
    if vessel_stats:
        ...
        print(
            f"{'Most common vessel class:':<25} {most_common['class_id']} ({most_common['fg_percentage']:.4f}% of foreground)"
        )
        print(
            f"{'Vessel class imbalance:':<25} {most_common['pixel_count']/max(rarest['pixel_count'],1):.2f}:1"
        )
...
def analyze_class_distribution(dataset, split_name, cache_path):
    ...
    if cache_path.exists():
        ...
        try:
            ...
            class_pixel_counts = {
                int(k): v for k, v in cached_data["class_pixel_counts"].items()
            }
            stats = cached_data["stats"]

            print(
                f"[INFO] Loaded from cache (generated at {cached_data['metadata']['generated_at']})\n"
            )
            ...
            print(
                f"[WARNING] Cache file corrupted ({e.__class__.__name__}), deleting and recomputing..."
            )
            cache_path.unlink()
    ...
    for cls_id in class_ids:
        ...
        if cls_id == 0:
            ...
            fg_percentage = (
                100 * count / foreground_pixels if foreground_pixels > 0 else 0.0
            )
        ...
        stats.append(
            {
                ...
                "fg_percentage": (
                    float(fg_percentage) if fg_percentage is not None else None
                ),
            }
        )
...
def get_combined_bbox(yolo_model, image_path, conf_threshold=0.25, img_size=1024):
    ...
    results = yolo_model.predict(
        source=str(image_path), imgsz=img_size, conf=conf_threshold, verbose=False
    )
...
def generate_bbox_cache(
    yolo_model_path, data_root, splits, conf_threshold, output_dir, img_size=1024
):
    ...
    for split in splits:
        ...
        image_files = sorted(
            list(split_dir.glob("*.png")) + list(split_dir.glob("*.jpg"))
        )
...
def run_pipeline(config: Config):
    ...
    # Save CLAHE debug samples (if enabled)
    # TODO: Move to config
    if USE_CLAHE:
        save_clahe_debug_samples(train_dataset, config.medsam_workdir, num_samples=3)

    train_stats, train_counts = analyze_class_distribution(
        train_dataset,
        "train",
        cache_path=Path(config.medsam_workdir)
        / "class_dist_cache"
    ...
    print(
        f"   [INFO] Semantic head parameters: {sum(p.numel() for p in medsam_syntax.semantic_head.parameters()):,}"
    )
    ...
    print(
        f"   [INFO] Trainable parameters: {sum(p.numel() for p in trainable_params):,}"
    )
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if (epoch + 1) % VAL_FREQ == 0:
            ...
            print(
                f"   [INFO] Validation: Loss={avg_val_loss:.4f}, mIoU={val_metrics_epoch['mean_iou']:.3f}, mF1={val_metrics_epoch['mean_f1']:.3f}, Acc={val_metrics_epoch['accuracy']:.3f}"
            )
