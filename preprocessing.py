...
def explore_clahe_parameters(image_path: str):
    ...
    for tile_size in tile_sizes:
        for clip_limit in clip_limits:
            ...
            axes[idx].set_title(
                f"CLAHE\nclip={clip_limit}, tile={tile_size}", fontsize=11
            )
            axes[idx].axis("off")

            results.append(
                {"image": enhanced, "clip_limit": clip_limit, "tile_size": tile_size}
            )
    ...
    for i, result in enumerate(results):
        filename = (
            output_dir
            / f"{image_path.stem}_clip{result['clip_limit']}_tile{result['tile_size'][0]}x{result['tile_size'][1]}.png"
        )
